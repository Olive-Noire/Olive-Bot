    ...
    if (message.member.roles.cache.has(message.guild.roles.cache.find(role => role.name === 'admin').id) || message.author.id === message.guild.ownerID || !message.member.hasPermission("BAN_MEMBERS")) {
        ...
        if (args[0] != null) {
            ...
            if (MuteUser == User) {
                ...
                message.delete();
                message.author.send("Vous ne pouvez pas vous mute `VOUS MEME` ou `EVERYONE`");
                return;
            ...
            if (MuteUser.id == message.guild.ownerID) {
                ...
                message.delete();
                message.author.send("Vous ne pouvez pas mute cette personne il s'agit du `PROPRIETAIRE DU SERVEUR 👑 !!`");
                return;
            ...
            message.guild.members.cache.get(MuteUser.id).send("Vous avez était mute\nraison : " + reason)

    
            message.author.send("La mute a bien était `AFFECTER ✅`\n\nVous avez mute **" + MuteUser.name + "**\n\n`RAISON AFFECTER :` " + reason);
